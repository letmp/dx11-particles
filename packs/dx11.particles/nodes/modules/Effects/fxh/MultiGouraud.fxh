...
float4 MultiGouraudDirectional(float3 NormV, float3 ViewDirV,float4x4 tV)
{
    ...
    for(int i = 0; i < lDirCount; i++){
        ...
        color += diff + lAmb + spec;
    }

    return color;
}

float4 MultiGouraudPoint(float3 PosW, float3 NormV, float3 ViewDirV, float4x4 tV)
{
